    ...
    public class CSharp_Dal : TemplateBase
    {
        ...
        public override void ProduceCode()
        {
            ...
            foreach (ColumnSchema column in Table.Columns())
            {
                ...
                if (column.Name.ToLower() == "chainid" && (column.Comment == null || column.Comment.ToLower() != "noscript"))
                    HasChainID = true;
            ...
            int cnt = 0;
            string dataClass = string.Format("{0}.BusinessEntities.{1}Info", base.Domain.Description, Table.Name);
            ...
            //WriteLine();
            WriteLine("using {0}.BusinessEntities;", base.Domain.Description);
            WriteLine();
            WriteLine("namespace {0}.DataAccess", base.Domain.Description);
            WriteLine("{");
        ...
        public {0}Dal(SqlConnection conn, SqlTransaction trans) : base(conn, trans) {{ }}
            ...
            WriteLine();
            WriteLine(@"
        public {2} Insert( {3}.BusinessEntities.{0}Info {1} )
		{{
			SqlParameter[] Param_Insert = GetParameters_Insert();", Table.Name, Table.Name.ToLower(), (ReturnIdentity) ? "int" : "void", base.Domain.Description);
            cnt = 0;
            foreach (ColumnSchema column in Table.Columns())
            {
            ...
            if (Table.PrimaryKeyColumns().Count > 0)
            {
                ...
                WriteLine();
                WriteLine(@"
        public int Update( {0}.BusinessEntities.{1}Info {2} )
		{{
			SqlParameter[] Param_Update = GetParameters_Update();", base.Domain.Description, Table.Name, Table.Name.ToLower());
                cnt = 0;
                foreach (ColumnSchema column in Table.Columns())
                {
                    if (column.NetDataType == "System.String")
        ...
        {{
            ...
            SqlParameter Param = Utils.GetParameters_HotelID();
			Param.Value = hotelID;

            return SQLHelper.ExecuteNonQuerySingleParm(base._internalConnection, base._internalADOTransaction, CommandType.Text, sql, Param);
        }}", Table.Name);
                WriteLine();
            }
        ...
        {{
            ...
            if (HasHotelID)
            {
                ...
                if (HasActive)
                    WriteLine(@"
        public List<{1}.BusinessEntities.{0}Info> SelectByHotel(int hotelID, bool activeOnly)
        {{
            List<{1}.BusinessEntities.{0}Info> ret = null;

            SqlParameter[] Params = Utils.GetParameters_HotelIDActiveOnly();
            ...
            {{
                ret = {1}.BusinessEntities.{0}Info.LoadDbRecords(rdr);
            }}
            return ret;
        }}", Table.Name, base.Domain.Description);
                else
                    WriteLine(@"
        public List<{1}.BusinessEntities.{0}Info> SelectByHotel(int hotelID)
        {{
            List<{1}.BusinessEntities.{0}Info> ret = null;
            ...
            using (SqlDataReader rdr = SQLHelper.ExecuteReaderSingleParm(base._internalConnection, base._internalADOTransaction, CommandType.StoredProcedure, SQL_SELECTBYHOTEL, Param)) 
            {{
                ret = {1}.BusinessEntities.{0}Info.LoadDbRecords(rdr);
            }}
            return ret;
            ...
            #region Select By Chain

            if (HasHotelID && HasChainID)
            {
                WriteLine();
                if (HasActive)
                    WriteLine(@"
        public List<{2}.BusinessEntities.{0}Info> SelectByChain(int chainID, bool activeOnly)
        {{
            const string sql = ""SELECT "" + {0}Info.SQLSELECT + "" FROM {0} {1} WHERE {1}.ChainID=@ChainID AND {1}.HotelID=0 AND ({1}.Active=@Active OR {1}.Active=1)"";
            List<{2}.BusinessEntities.{0}Info> ret = null;

            SqlParameter[] Params = Utils.GetParameters_ChainIDActiveOnly();
			Params[0].Value = chainID;
			Params[1].Value = activeOnly;
            ...
            {{
                ret = {2}.BusinessEntities.{0}Info.LoadDbRecords(rdr);
            }}
            return ret;
        }}", Table.Name, tbInitial, base.Domain.Description);
                else
                    WriteLine(@"
        public List<{2}.BusinessEntities.{0}Info> SelectByChain(int chainID)
        {{
            const string sql = ""SELECT "" + {0}Info.SQLSELECT + "" FROM {0} {1} WHERE {1}.ChainID=@ChainID AND {1}.HotelID=0"";
            List<{2}.BusinessEntities.{0}Info> ret = null;

            SqlParameter[] Params = Utils.GetParameters_ChainID();
			Params[0].Value = chainID;

            using (SqlDataReader rdr = SQLHelper.ExecuteReaderSingleParm(base._internalConnection, base._internalADOTransaction, CommandType.Text, sql, Params[0])) 
            {{
                ret = {2}.BusinessEntities.{0}Info.LoadDbRecords(rdr);
            }}
            return ret;
            ...
            if (Table.PrimaryKeyColumns().Count > 0)
            {
                WriteLine();
                WriteLine(@"
        public {3}.BusinessEntities.{0}Info Select({1})
        {{
            {3}.BusinessEntities.{0}Info ret = null;
            ...
            {{
                ret = {3}.BusinessEntities.{0}Info.LoadDbRecord(rdr);
            }}
            return ret;
        }}", Table.Name, InputParams, ParameterAssigment, base.Domain.Description);
                WriteLine();
            }
...
//            #endregion
        ...
        private string BuildParamStatement(ColumnSchema column)
        {
            ...
            switch (column.OriginalSQLType)
            {
                ...
                case "decimal":
                    return string.Format("{0}{1},                 {2}),\n", FirstPart, column.Name.ToUpper(), "SqlDbType.Decimal");
                default:
