...
namespace SmartCode.Database
{
    public class DataInfoDB : TemplateBase
    {
        ...
        public override void ProduceCode()
        {
            ...
            WriteLine();
            WriteLine("namespace {0}.BusinessEntities", base.Domain.Description);
            WriteLine("{");
            WriteLine();
            ...
            foreach (ColumnSchema column in Table.Columns())
            {
                ...
                {
                    WriteLine("             if ( rdr.IsDBNull({0}) )", cnt);
                    WriteLine("                    {0}", BuildIsNullStatement( column ));
                    WriteLine("             else");
        ...
        private string BuildIsNullStatement(ColumnSchema column)
        {
            switch (column.OriginalSQLType)
            {
                ...
                case "decimal":
                case "numeric":
                    return string.Format("                obj.{0} = 0;", column.Name);
                case "datetime":
                case "smalldatetime":
                    return string.Format("                obj.{0} = DateTime.MinValue;", column.Name);
                    break;
                case "char":
                case "nchar":
                case "varchar":
                ...
                case "text":
                case "ntext":
                    return string.Format("                obj.{0} = string.Empty;", column.Name);
                default:
                    return string.Format("                obj.{0} = null;", column.Name);
                    break;
            }
            return "";
        }
