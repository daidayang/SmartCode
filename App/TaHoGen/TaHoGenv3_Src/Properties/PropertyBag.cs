...
namespace TaHoGen
{
	...
	public class PropertyBag : ICustomTypeDescriptor
	{
		...
		{
			// Search for the property that matches that name
			foreach(PropertySpec ps in properties)
			{
				if (ps.Name == propertyName)
					return GetPropertyValue(ps);
			}

			return null;
		}
		internal object GetPropertyValue(PropertySpec property)
		{
			PropertySpecEventArgs args = new PropertySpecEventArgs(property, null);
			if (GetValue != null)
				GetValue(this, args);
	...
	public class PropertyTable : PropertyBag
	{
		...
		{
			this.GetValue +=new PropertySpecEventHandler(PropertyTable_GetValue);
			this.SetValue +=new PropertySpecEventHandler(PropertyTable_SetValue);
		...
		public object this[string key]
		{
			...
			{ 
				...
				if (base.HasProperty(key) == false)
				{
					PropertySpec spec = new PropertySpec(key, value.GetType());
					Properties.Add(spec);
				}
		...
		/// <summary>
		/// Clears the values in the property table.
		/// </summary>
		public void Clear()
		{
			propValues.Clear();
		...
		{
			return propValues.ContainsKey(propertyName);
		}

		private void PropertyTable_GetValue(object sender, PropertySpecEventArgs e)
		{
			e.Value = this[e.Property.Name];
		}

		private void PropertyTable_SetValue(object sender, PropertySpecEventArgs e)
		{
